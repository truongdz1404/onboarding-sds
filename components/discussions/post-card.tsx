...
export function PostCard({ post, userVote: initialUserVote = null, onArchived }: PostCardProps) {
  ...
  useEffect(() => { setVoteCount(post.upvoteCount) }, [post.upvoteCount])
  ...
  return (
    <div className="group relative cursor-pointer rounded-xl bg-white px-4 py-3 my-1 transition-colors duration-150 hover:bg-[#f6f7f8]">
      ...
      <Link className="absolute inset-0 rounded-xl" href={`/discussions/${post.id}`} aria-label={post.title} />
      ...
      <div className="relative flex flex-col gap-2">
        ...
        <div className="flex flex-wrap items-center gap-x-1.5 gap-y-1 text-sm text-gray-500">
          ...
          <span className={cn('font-semibold text-gray-800')}>p/{meta.slug}</span>
        ...
        <div className="relative z-10 flex items-center gap-2 pt-1">
          ...
          <div className="flex items-center overflow-hidden rounded-full border border-gray-200 bg-white text-sm font-semibold text-gray-700">
            ...
            <span className={cn(
              'px-1 tabular-nums',
              userVote === 'up' ? 'text-primary' : userVote === 'down' ? 'text-red-500' : 'text-gray-700',
            )}>
              {voteCount}
            </span>
            <button

...
export default function BlogListClient({
  ...
  // Firebase takes precedence over static for duplicate slugs
  const firebaseSlugs = new Set(firebasePosts.map(p => p.slug))
  const allPosts = [
    ...POSTS.filter(p => !firebaseSlugs.has(p.slug)),
    ...firebasePosts,
  ]
  const visible =
    filter === 'Tất cả' ? allPosts : allPosts.filter((post) => post.category === filter)

  // Use Firebase version of featured post if it exists (may have coverImage)
  const featuredPost = firebasePosts.find(p => p.slug === FEATURED.slug) ?? FEATURED
  const featuredViews = viewCounts[FEATURED.slug] > 0 ? viewCounts[FEATURED.slug] : featuredPost.views
        ...
        <div className="mx-auto max-w-7xl px-5 lg:px-8">
              ...
              <div>
                ...
                <div className="mt-5 flex items-center gap-3 text-sm text-muted-foreground">
                  ...
                  <div>
                    ...
                    <div>{FEATURED.readTime} đọc · {featuredViews} lượt xem</div>
                  </div>
                </div>
          ...
          <motion.div layout className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            <AnimatePresence mode="popLayout">
              {visible.map((post, i) => (
                <BlogCard key={post.slug} post={post} index={i} viewCount={viewCounts[post.slug]} />
              ))}
            </AnimatePresence>
          </motion.div>
